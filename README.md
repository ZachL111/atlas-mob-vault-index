...
atlas-mob-vault-index is a Go project for mobile workflows. It focuses on this technical goal: Create a Go reference implementation for vault workflows, centered on storage recovery, log and snapshot fixtures, and replay consistency checks.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 155, risk penalty 4, latency penalty 3, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
