...
`atlas-mob-vault-index` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for vault workflows, centered on storage recovery, log and snapshot fixtures, and replay consistency checks.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 3.
